...
function getNick() {
  ...
  if (fromInput) { localStorage.setItem(NICK_LS, fromInput); return fromInput; }
...
/* =========================
   HRA (původní logika + drobné doplnění)
   ========================= */
const W = 700, H = 300;
...
try { const s = JSON.parse(localStorage.getItem(LS)); if (s && typeof s==="object") best = {...best, ...s}; } catch {}
...
function renderMe(meEl, top3Rows, myBest) {
  ...
  if (iAmInTop3) meEl.textContent = ""; else meEl.textContent = (myBest ?? "—");
}
