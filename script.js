(() => {
    ...
    if (nickBtn) {
        nickBtn.onclick = async () => {
            ...
            if (!n) {
                ...
                } catch (e) { console.error("Delete on empty nick failed:", e); }
                return;
            }
            try {
                setNick(n);
                ...
                await renderLeaderboard();
            } catch (e) { console.error("Save nick failed:", e); }
        };
    }
    ...
    let headVisible = false;     // hlava zobrazená?
...
`;
    ...
    function render() {
        ...
        if (headVisible && head.complete) {
            ...
            x.rect(ix, iy - 20, iw, Math.floor(ih * 0.33-20));   // uprav podle výšky vajíčka
            x.clip();
    ...
    async function triggerSlice() {
        ...
        if (cut === null) {
            ...
            if (Math.abs(r - SV) <= TOL) {
                ...
                // >>> HEAD FEATURE — spustit animaci vysouvání hlavy
                headVisible = true;
                headSliding = true;
                headSlide = 0;
    ...
    (async () => {
        const n = getNick();
        if (n) {
            try { await ensureAllDifficultiesUpsert(n); } catch { }
        }
    })();
